class CountdownApp {
    ...
    init() {
        document.getElementById('addBtn').addEventListener('click', () => this.addEvent());
    ...
    initTypeButtons() {
        ...
        typeButtons.forEach(btn => {
            btn.addEventListener('click', (e) => {
                document.querySelectorAll('.type-btn').forEach(b => b.removeAttribute('selected'));
    ...
    stopDrag() {
        ...
        const transform = parseInt(this.dragging.style.transform.match(/-?\d+/)[0]);
        const maxValue = parseInt(this.dragging.dataset.maxValue);
        ...
        let snappedValue;
        if (topItemValue > bottomItemValue) {
            snappedValue = topItemValue - Math.round(transform / this.itemHeight);
        } else {
            snappedValue = Math.round(-transform / this.itemHeight);
        }
        
        const clampedValue = Math.max(0, Math.min(maxValue, snappedValue));
        this.dragging.dataset.value = clampedValue;
        ...
        this.dragging.style.transform = `translateY(-${transformValue}px)`;
        this.dragging = null;
    }
